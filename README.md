...
#### 使用 Android 输入法 ([SyncClipboard Android IME](src/SyncClipboard.AndroidIME/README.md))

一个独立的 Android 输入法实现，可以实现电脑复制、平板直接粘贴的功能：

- **输入法集成**：作为 Android 输入法运行，可以在任何应用中获取剪贴板内容
- **无感知同步**：激活输入法时自动检查并同步剪贴板内容
- **自动同步**：支持自动上传本地剪贴板和自动下载远程剪贴板
- **轻量级**：独立的输入法实现，不依赖第三方应用

详见 [Android IME 文档](src/SyncClipboard.AndroidIME/README.md)
